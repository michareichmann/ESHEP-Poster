...
\DeclareDocumentCommand{\nicetab}{m m O{nocap} O{nolab}}{
	\begin{table}[ht!]\centering\setlength\extrarowheight{2pt}\alternatecolors
		\begin{tabular}{#1}\rowcolor{darkgray!30}\tline{.5} #2
			\tline{.5}
		\end{tabular}
		\caplab{#3}{#4}
	\end{table}}
\newcommand{\downarrows}[2]{\begin{minipage}{.23\linewidth} \fig{.03}{#2} \end{minipage}
		\begin{minipage}{.5\linewidth}\centering
			\usebeamercolor{title in headline}{\color{fg}\textbf{\huge #1}}
		\end{minipage}
		\begin{minipage}{.23\linewidth} \fig{.03}{#2} \end{minipage}}
